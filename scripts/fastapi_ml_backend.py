...
# class TransferRequestOptional(BaseModel):
#     patient_id: str
#     nurse_id: str
...
# # Function to get database connection (you need to implement this or use your existing db connection method)
# def get_connection():
...
# # Transfer request endpoints - FIXED VERSION
# # Debug Transfer request endpoint - Let's see what's failing
# @app.post("/transfer-request")
# async def create_transfer_request(request_data: dict):  # Changed from TransferRequest to dict
#     try:
...
# # In-memory storage for transfer requests (since database might not be properly set up)
# transfer_requests = {}
...
#     try:
#         while True:
#             # Send real-time patient updates every 30 seconds
#             await asyncio.sleep(30)
...
#                 # Send static vitals from database
#                 vitals = {
#                     "patient_id": patient['id'],
#                     "heartRate": patient['vitals']['heartRate'],
#                     "spO2": patient['vitals']['spO2'],
...
# # FIXED Transfer request endpoints with proper UUID
# @app.post("/transfer-request")
...
#         # Generate unique request_id - FIXED
#         request_id = f"TR-{uuid.uuid4().hex[:8]}"
...
#         # Store in in-memory dictionary AND try database
#         transfer_requests[request_id] = transfer_request_record
        
#         # Try to also store in database if available
#         try:
...
#         # FIXED: Notify all WebSocket clients about the new transfer request
#         notification = {
...
#             # Also include in-memory requests that might not be in database
#             all_requests = list(db_requests)
#             for req_id, req_data in transfer_requests.items():
#                 # Check if this request is already in database results
...
#         # Try to also update in database
#         try:
#             success = db.update_transfer_request(request_id, update_data)
#             if success:
...
#         # FIXED: Notify WebSocket clients about the update
#         notification = {
#             "type": "transfer_request_updated", 
#             "data": updated_request
#         }
...
# # NEW: Reject transfer request endpoint
...
# # NEW: Approve transfer request endpoint  
...
#     """Get all departments with capacity information"""
#     try:
#         # Fallback departments if database fails
#         default_departments = [
